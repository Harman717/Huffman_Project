...
class CustNode {
  ...
  static getRevCompareFunction() {
    return function (arg0, arg1) {
      return arg1.nodeValue - arg0.nodeValue;
    };
  }
  ...
  updateDepth(depth) {
    ...
    if (this.nodeRight != null) this.nodeRight.updateDepth(this.depth + 1);

    if (this.nodeLeft != null) this.nodeLeft.updateDepth(this.depth + 1);
  }
  ...
  toHtml() {
    if (this.nodeType == "L")
      return (
        "<span class='node_view_L'><div class='node_label'>" +
        this.nodeLabel +
        "</div><div class='node_value'>" +
        this.nodeValue +
        "</div></span>"
      );
    else
      return (
        "<span class='node_view_I'><div class='node_value'>" +
        this.nodeValue +
        "</div></span>"
      );
  }
...
class TreeUtil {
  ...
  static genSVG() {
    ...
    for (let i = 0; i < TreeUtil.inorderTraversal.length; i++) {
      ...
      if (node.nodeType == "I") {
        TreeUtil.svg +=
          ...
          (i + 1) * TreeUtil.scale +
          'px" y1="' +
          (node.depth + 1) * TreeUtil.scale +
          'px" x2="' +
          (TreeUtil.inorderTraversal.indexOf(node.nodeLeft) + 1) *
            TreeUtil.scale +
          'px" y2="' +
          (node.depth + 2) * TreeUtil.scale +
          'px" />';
        TreeUtil.svg +=
          '<line stroke="black" x1="' +
          (i + 1) * TreeUtil.scale +
          ...
          (node.depth + 1) * TreeUtil.scale +
          'px" x2="' +
          (TreeUtil.inorderTraversal.indexOf(node.nodeRight) + 1) *
            TreeUtil.scale +
          'px" y2="' +
          (node.depth + 2) * TreeUtil.scale +
          'px" />';
    ...
    for (let i = 0; i < TreeUtil.inorderTraversal.length; i++) {
      ...
      if (node.nodeType == "L") {
        TreeUtil.svg +=
          '<rect fill="coral" stroke="black" stroke-width="0.5px" x="' +
          ((i + 1) * TreeUtil.scale - 5) +
          'px" y="' +
          ((node.depth + 1) * TreeUtil.scale - 5) +
          'px" width="10px" height="10px" rx="2px" ry="2px"/>';
        TreeUtil.svg +=
          '<text font-family="monospace" fill="white" font-size="4px" x="' +
          ((i + 1) * TreeUtil.scale - 2) +
          'px" y="' +
          ((node.depth + 1) * TreeUtil.scale - 1) +
          'px">' +
          node.nodeLabel +
          "</text>";
        TreeUtil.svg +=
          ...
          ((i + 1) * TreeUtil.scale - 2) +
          'px" y="' +
          ((node.depth + 1) * TreeUtil.scale + 4) +
          'px">' +
          node.nodeValue +
          "</text>";
        TreeUtil.svg +=
          '<line stroke="white" stroke-width="0.5px" x1="' +
          ((i + 1) * TreeUtil.scale - 3) +
          'px" y1="' +
          (node.depth + 1) * TreeUtil.scale +
          'px" x2="' +
          ((i + 1) * TreeUtil.scale + 3) +
        ...
        TreeUtil.svg +=
          '<circle fill="coral" stroke="black" stroke-width="0.5px" cx="' +
          (i + 1) * TreeUtil.scale +
          'px" cy="' +
          (node.depth + 1) * TreeUtil.scale +
          'px" r="5px"/>';
        TreeUtil.svg +=
          '<text font-family="monospace" fill="white" font-size="6px" x="' +
          ((i + 1) * TreeUtil.scale - 2) +
          'px" y="' +
          ((node.depth + 1) * TreeUtil.scale + 2) +
          'px">' +
          node.nodeValue +
          "</text>";
      }
  ...
  static getSVG() {
    return (
      '<svg id="forestView" viewBox="0 0 ' +
      (TreeUtil.inorderTraversal.length + 2) * TreeUtil.scale +
      " " +
      (TreeUtil.maxDepth + 2) * TreeUtil.scale +
      '">' +
      TreeUtil.svg +
      "</svg>"
    );
  ...
  static drawForest(forest) {
    TreeUtil.maxDepth = 0;
    for (let node of forest) node.updateDepth(0);
...
function generateExplanation(labels, values, isHuff = false) {
  ...
  while (list.nodes.length > 1) {
    ...
    output +=
      "Now pick two nodes with minimum values (i.e. " +
      nodeLeft.toHtml() +
      " and " +
      nodeRight.toHtml() +
      ")<br>Find the sum of its values (i.e. <b>" +
      nodeLeft.nodeValue +
      " + " +
      nodeRight.nodeValue +
      " = " +
      newNode.nodeValue +
      "</b>) and put the sum in a new Intermediate Node. Now the new node created is " +
      newNode.toHtml() +
      "<br>Make the two nodes its child. Such that the node with smaller value(here " +
      nodeLeft.toHtml() +
      ") is its left child and one with bigger value(here " +
      nodeRight.toHtml() +
      ") is its right child.<br><br><br> Now our new node is,<br>";

    TreeUtil.drawTree(newNode);
    output +=
      TreeUtil.getSVG() +
      "<br><br><br>Finally push it into the array and we get,";
  ...
  output +=
    "<br><hr><br/><h2>Final " +
    (isHuff ? "Huffman" : "Optimal Merge Pattern") +
    " Tree</h2>";
  ...
  let resultTable =
    "<table><tr>" +
    (isHuff
      ? "<th>Character</th><th>Frequency</th><th>Huffman Code</th><th>Code Length</th><th>Weighted Code Length</th>"
      : "<th>File Name</th><th>Size</th><th>Path Length</th><th>Weighted Path Length</th>") +
    "</tr>";
  ...
  for (let node of TreeUtil.inorderTraversal) {
    if (node.nodeType == "L")
      resultTable +=
        ...
        node.nodeValue +
        "</td>" +
        (isHuff ? "<td>" + node.huffCode + "</td>" : "") +
        "<td>" +
        node.huffCode.length +
        "</td><td>" +
        node.huffCode.length * node.nodeValue +
        "</td></tr>";
  ...
  resultTable +=
    ...
    (isHuff ? "Code" : "Path") +
    " Length</td><td>" +
    weighted_external_path_length +
    "</td></tr></table>";

  document.getElementById("output").innerHTML +=
    "<br><hr><h2>Resulting Data Table</h2>" + resultTable;
}
...
function generateList() {
  ...
  list.innerHTML =
    '<div class="label">' +
    (isHuff ? "Character" : "File Name") +
    '</div><div class="label">' +
    (isHuff ? "Frequency" : "Size") +
    "</div>";
  ...
  list.innerHTML +=
    '<input type="button" value="Explain" onclick="explain()"></div>';
}
